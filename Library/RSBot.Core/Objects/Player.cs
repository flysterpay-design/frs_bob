...
public class Player : SpawnedBionic
{
    ...
    private int _lastDismountTick;
    private bool _dismountRequested;
    ...
    /// <summary>
    ///     Слезает с транспорта, если расстояние до центра меньше заданного порога.
    ///     Выполняет повторные попытки, если транспорт не слез сразу.
    /// </summary>
    /// <param name="center">Центр зоны (обычно Area.Position)</param>
    /// <param name="distanceThreshold">Максимальное расстояние для начала слезания (по умолчанию 30)</param>
    public void EnsureDismount(Position center, int distanceThreshold = 30)
    {
        if (!HasActiveVehicle)
        {
        ...
        double distance = Position.DistanceTo(center);  // ← double вместо float
        if (distance > distanceThreshold)
            return;

        int now = Kernel.TickCount;

        if (!_dismountRequested)
        {
            Log.Status("Dismounting transport near the center...");
            Vehicle.Dismount();
            _dismountRequested = true;
            _lastDismountTick = now;
        }
        else if (_dismountRequested && HasActiveVehicle && now - _lastDismountTick > 1000)
        {
            Log.Debug("Retrying dismount...");
            Vehicle.Dismount();
            _lastDismountTick = now;
        }
        else if (_dismountRequested && !HasActiveVehicle)
        {
            _dismountRequested = false;
        }
    }
