...
public class ScriptManager
{
    ...
    public static bool IsTownScriptRunning { get; set; }

    /// <summary>
    ///     Timeout in milliseconds for each script command (default 20 seconds)
    /// </summary>
    public static int CommandTimeoutMs { get; set; } = 40000;
    ...
    public static void RunScript(bool useNearbyWaypoint = true, bool ignoreBotRunning = false)
    {
        ...
        foreach (var scriptLine in Commands?.Skip(CurrentLineIndex))
        {
            ...
            // --- ИЗМЕНЕНИЕ: выполнение команды с таймаутом ---
            var executionResult = ExecuteCommandWithTimeout(handler, arguments.Skip(1).ToArray(), CommandTimeoutMs);
            ...
            {
                LogScriptMessage(
                    "The execution of the script command failed or timed out.",
                    CurrentLineIndex,
                    LogLevel.Warning,
                    commandName
                );

                // При ошибке или таймауте останавливаем скрипт с пометкой ошибки
                error = true;
                break;
            }
    ...
    /// <summary>
    ///     Executes a script command with a timeout.
    /// </summary>
    private static bool ExecuteCommandWithTimeout(IScriptCommand handler, string[] args, int timeoutMs)
    {
        try
        {
            var task = Task.Run(() => handler.Execute(args));
            if (task.Wait(timeoutMs))
                return task.Result;

            LogScriptMessage($"Command '{handler.Name}' timed out after {timeoutMs} ms", CurrentLineIndex, LogLevel.Warning, handler.Name);
            handler.Stop(); // Attempt to stop the hanging command
            return false;
        }
        catch (Exception ex)
        {
            LogScriptMessage($"Exception in command '{handler.Name}': {ex.Message}", CurrentLineIndex, LogLevel.Error, handler.Name);
            handler.Stop();
            return false;
        }
    }
