...
public partial class Main : DoubleBufferedControl
{
    ...
    // Новый элемент для радиуса баффов партии
    private SDUI.Controls.NumUpDown numPartyBuffRange;
    private SDUI.Controls.Label lblPartyBuffRange;
    ...
    private void InitializeBuffRangeControl()
    {
        // Панель, которая будет внизу groupBox1
        var rangePanel = new SDUI.Controls.Panel
        {
        ...
        lblPartyBuffRange = new SDUI.Controls.Label
        {
            Text = "Party buff range (m):",
            Location = new Point(12, 12),
            AutoSize = true,
            ForeColor = Color.Black
        };

        numPartyBuffRange = new SDUI.Controls.NumUpDown
        {
            Minimum = 10,
            Maximum = 200,
            Value = 50,
        ...
        rangePanel.Controls.Add(lblPartyBuffRange);
        rangePanel.Controls.Add(numPartyBuffRange);

        // Добавляем панель в groupBox1 (контейнер списка баффов)
        groupBox1.Controls.Add(rangePanel);
    }
    ...
    private void LoadSettings()
    {
        ...
        _applySettings = true;
        numPartyBuffRange.Value = PlayerConfig.Get("RSBot.Party.BuffRange", 50);
    }
    ...
    private void LoadPartyBuffSkills()
    {
        ...
        // Все активные, не пассивные, не атакующие и не воскрешения (лечение, баффы, массовые)
        var skills = Game.Player.Skills.KnownSkills.Where(s =>
            s.Enabled &&
            !s.IsPassive &&
            !s.IsAttack &&                     // исключаем атакующие скиллы
            !s.Record.TargetEtc_SelectDeadBody
        );

        // Дополнительные группы для некоторых баффов (можно оставить, но они уже не нужны)
        List<string> additionalGroups = new List<string>
    {
        "SKILL_EU_BARD_SPEEDUPA_MSPEED_A",
        "SKILL_EU_BARD_SPEEDUPA_MSPEED_B",
        "SKILL_EU_CLERIC_SAINTA_ABNORMAL_B",
        "SKILL_EU_CLERIC_SAINTA_ABNORMAL_B_1",
    };
        ...
        var partyBuffSkills = skills.Union(additionalSkills);
        ...
        foreach (var skill in partyBuffSkills)
        {
            ...
            // Лимит использования (если есть)
            int limit = 0;
            int paramIndex = skill.Record.Params.FindIndex(p => p == 1819175795);
            if (paramIndex != -1)
                limit = skill.Record.Params[paramIndex + 3];

            int count = _buffings.Count(p => p.Group == _selectedBuffingGroup.Text && p.Buffs.Any(v => v == skill.Id));
            ...
            // Привязка к группе мастерства
            foreach (var group in listPartyBuffSkills.Groups.Cast<ListViewGroup>().Where(g => Convert.ToInt32(g.Tag) == skill.Record.ReqCommon_Mastery1))
                item.Group = group;
    ...
    {
        LoadSettings();
        numPartyBuffRange.ValueChanged += (s, ev) =>
        {
            if (!_applySettings) return;
            PlayerConfig.Set("RSBot.Party.BuffRange", (int)numPartyBuffRange.Value);
            PlayerConfig.Save();
        };
    }
}
