...
internal class LoopConditionValidator
{
    public static string CheckLoopConditions()
    {
        ...
        if (!CheckNumPartyMemberDead())
            return $"[Lure] Pausing lure bot because there are more than {LureConfig.NumPartyMemberDead} people in the party are dead.";
        ...
        if (!CheckNumMonsterType())
            return $"[Lure] Pausing lure bot because there are more than {LureConfig.NumMonsterType} monsters of selected type around.";

        // НОВАЯ ПРОВЕРКА
        if (!CheckTotalMonsters())
            return $"[Lure] Pausing lure bot because there are more than {LureConfig.TotalMonstersCount} total monsters around.";

        return null;
    }

    // НОВЫЙ МЕТОД
    private static bool CheckTotalMonsters()
    {
        if (!LureConfig.StopIfTotalMonsters)
            return true;

        if (!SpawnManager.TryGetEntities<SpawnedMonster>(
                m => m.Position.DistanceTo(LureConfig.Area.Position) <= 15,
                out var mobs
            ))
            return true;

        return mobs.Count() < LureConfig.TotalMonstersCount;
    }
    ...
    private static bool CheckNumMonsterType()
    {
        ...
        if (
            !SpawnManager.TryGetEntities<SpawnedMonster>(
                m =>
                    m.Rarity >= LureConfig.SelectedMonsterType && m.Position.DistanceTo(LureConfig.Area.Position) <= 15,
                out var mobs
            )
        )
            return true;
